        ...
        <div class="col-md-6 col-lg-3">
            <div class="stat-card-sm">
                ...
                <div>
                    ...
                    <div class="stat-value-sm">{{ $stats['total_today'] ?? 24 }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-6 col-lg-3">
            <div class="stat-card-sm">
                ...
                <div>
                    ...
                    <div class="stat-value-sm">{{ $stats['confirmed'] ?? 18 }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-6 col-lg-3">
            <div class="stat-card-sm">
                ...
                <div>
                    ...
                    <div class="stat-value-sm">{{ $stats['pending'] ?? 6 }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-6 col-lg-3">
            <div class="stat-card-sm">
                ...
                <div>
                    ...
                    <div class="stat-value-sm">${{ number_format($stats['revenue_today'] ?? 2840) }}</div>
                </div>
            </div>
        </div>
            ...
            <table class="table-custom" id="appointmentsTable">
                ...
                <tbody>
                    ...
                    @foreach ($appointments as $appt)
                        <tr class="appt-row"
                            data-client="{{ strtolower($appt['client_name']) }}"
                            data-status="{{ strtolower($appt['status']) }}"
                            data-stylist="{{ strtolower($appt['stylist']) }}">
                            <td>
                                <div class="cell-name">{{ $appt['client_name'] }}</div>
                                <div class="cell-sub">{{ $appt['client_email'] }}</div>
                                <div class="cell-sub">{{ $appt['client_phone'] }}</div>
                            </td>
                            <td>{{ $appt['service'] }}</td>
                            <td>
                                <div class="cell-name">{{ $appt['date'] }}</div>
                                <div class="cell-sub">{{ $appt['time_range'] }}</div>
                            </td>
                            <td>{{ $appt['stylist'] }}</td>
                            <td class="amount-gold">${{ $appt['price'] }}</td>
                            <td>
    ...
    <div class="modal fade" id="deleteApptModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content modal-content-custom">
                <form action="{{ route('owner.appointments.destroy', ['appointment' => 0]) }}" method="POST" id="deleteApptForm">
                    @csrf
                    @method('DELETE')
...
<style>
    ...
    .btn-add-appt:hover { transform: translateY(-1px); box-shadow: 0 6px 18px rgba(217, 164, 65, 0.5); color: var(--plum-900); }
...
<script>
    ...
    document.querySelectorAll('.delete-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            form.action = form.action.replace(/appointments\/\d+$/, 'appointments/' + this.dataset.id);
        });
    });
</script>
@endsection
