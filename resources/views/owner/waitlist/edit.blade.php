...
@section('title', 'Edit Waitlist Entry')
...
@section('content')
    ...
    <div class="page-header d-flex justify-content-between align-items-start flex-wrap gap-3">
        <div>
            ...
            <p>Update details for "{{ $entry['client_name'] }}"</p>
        </div>
    ...
    <form action="{{ route('owner.waitlist.update', ['waitlist' => $entry['id']]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="row g-4">
                        ...
                        <div class="col-12">
                            ...
                            <select name="service" class="form-select input-custom" required>
                                @foreach ($services ?? [] as $service)
                                    <option {{ $entry['service'] === $service ? 'selected' : '' }}>{{ $service }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12">
                            <label class="form-label-custom">Preferred Date</label>
                            <input type="date" name="preferred_date" class="form-control input-custom"
                                   value="{{ $entry['preferred_date_raw'] }}" required>
                        </div>
                        <div class="col-12">
                            <label class="form-label-custom">Priority</label>
                            <select name="priority" class="form-select input-custom" required>
                                @foreach (['Low', 'Medium', 'High'] as $p)
                                    <option {{ $entry['priority'] === $p ? 'selected' : '' }}>{{ $p }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <div class="col-12">
                <div class="panel-card">
                    ...
                    <textarea name="notes" class="form-control input-custom" rows="3">{{ $entry['notes'] ?? '' }}</textarea>
