...
@section('content')
    ...
    <div class="page-header d-flex justify-content-between align-items-start flex-wrap gap-3">
        <div>
            <h2>Add Review</h2>
            <p>Manually log feedback received from clients</p>
        </div>
    ...
    <form action="{{ route('owner.reviews.store') }}" method="POST">
        @csrf
 
        <div class="row g-4">
            <div class="col-lg-6">
                <div class="panel-card">
                    <div class="panel-title">Select Appointment</div>
 
                    <div class="row g-3">
                        <div class="col-12">
                            <label class="form-label-custom">Appointment <span class="text-danger">*</span></label>
                            <select name="appointment_id" class="form-select input-custom @error('appointment_id') is-invalid @enderror" required>
                                <option value="">Select an appointment</option>
                                @foreach ($appointments ?? [] as $appointment)
                                    <option value="{{ $appointment->id }}" {{ old('appointment_id') == $appointment->id ? 'selected' : '' }}>
                                        {{ $appointment->client->name ?? 'N/A' }} - {{ $appointment->service->name ?? 'N/A' }} ({{ $appointment->appointment_date ? date('M d, Y', strtotime($appointment->appointment_date)) : 'N/A' }})
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="panel-card">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="comment" class="form-control input-custom @error('comment') is-invalid @enderror" rows="4"
                                      placeholder="Write the client's feedback...">{{ old('comment') }}</textarea>
                            @error('comment')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="panel-card">
                    <div class="d-flex gap-3">
                        <button type="submit" class="btn btn-save-changes">
                            <i class="bi bi-check-circle-fill me-2"></i> Add Review
                        </button>
