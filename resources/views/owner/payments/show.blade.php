...
@push('modals')
    {{-- Reject Modal --}}
    @php
        // Quick reasons: owner ek click se message bhar sakta hai, ya khud likh sakta hai
        $quickReasons = [
            ...
            <div class="modal-content" style="border-radius:16px; border:none; overflow:hidden;">
                <form action="{{ route('owner.payments.reject', ['payment' => $payment['id']]) }}" method="POST">
                    ...
                    <div class="modal-body" style="padding:22px 24px;">
                        <label style="display:block; font-size:13.5px; font-weight:600; color:#5A4A55; margin-bottom:8px;">
                        ...
                        <div class="d-flex flex-wrap gap-2 mb-3">
                            @foreach ($quickReasons as $label => $text)
                                <button type="button"
                                        data-reason="{{ $text }}"
                                        onclick="document.getElementById('rejectReason').value = this.dataset.reason"
                                        style="background:#FFF0F6; color:#C0547A; border:1px solid #F0C0D8; border-radius:20px; padding:4px 12px; font-size:12.5px; font-weight:600; cursor:pointer;">
                                    {{ $label }}
                                </button>
                            @endforeach
                        </div>
 
                        <textarea id="rejectReason" name="reason" rows="3" required maxlength="255"
                                  ...
                                  placeholder="Choose a quick reason above or write your own..."></textarea>
                        <small style="color:#9E7B95; display:block; margin-top:6px;">
                            The client will see this reason in their notification and payment details.
                        </small>
                    </div>
