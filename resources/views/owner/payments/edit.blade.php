...
@section('content')
    ...
    <div class="page-header d-flex justify-content-between align-items-start flex-wrap gap-3">
        <div>
            <h2>Edit Payment</h2>
            <p>Update details for "{{ $payment['payment_id'] }}"</p>
        </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-6">
                            ...
                            <select name="method" class="form-select input-custom" required>
                                @foreach (['Credit Card', 'Debit Card', 'Cash'] as $m)
                                    <option {{ $payment['method'] === $m ? 'selected' : '' }}>{{ $m }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-6">
                            ...
                            <input type="date" name="date" class="form-control input-custom"
                                   value="{{ $payment['date_raw'] }}" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label-custom">Time</label>
                            <input type="time" name="time" class="form-control input-custom"
                                   value="{{ $payment['time_raw'] }}" required>
                        </div>
                        <div class="col-12">
                            <label class="form-label-custom">Status</label>
                            <select name="status" class="form-select input-custom">
                                @foreach (['Completed', 'Pending', 'Failed', 'Refunded'] as $s)
                                    <option {{ $payment['status'] === $s ? 'selected' : '' }}>{{ $s }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
