...
@section('content')
    ...
    <form action="{{ route('owner.profile.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row g-4">
            ...
            <!-- Left Column - Logo + Cover Image -->
            <div class="col-lg-4 d-flex">
                <div class="profile-left">
 
                    <!-- Logo Card -->
                    <div class="panel-card logo-card text-center">
                    ...
                    <!-- Cover Image Card -->
                    <div class="panel-card cover-card text-center">
                        <div class="panel-title mb-3">
                            <i class="bi bi-image me-2"></i> Cover Image
                        </div>
 
                        <div class="cover-preview" id="coverPreview">
                            @if (!empty($salon->cover_image))
                                <img src="{{ \Illuminate\Support\Str::startsWith($salon->cover_image, ['http://', 'https://'])
                                                ? $salon->cover_image
                                                : asset('storage/' . $salon->cover_image) }}"
                        ...
                        <div>
                            <label for="coverUpload" class="btn btn-change-logo mt-3">
                                <i class="bi bi-camera-fill me-2"></i>
                                {{ !empty($salon->cover_image) ? 'Change Cover Image' : 'Upload Cover Image' }}
                            </label>
                            <input type="file" id="coverUpload" name="cover_image" accept="image/*" hidden>
                            <div id="coverFileName" class="logo-filename"></div>
                            <small class="text-muted d-block mt-1">
                                This photo shows on the homepage and at the top of your salon page.
                                Max 4MB (JPG, PNG, WEBP). Click "Save Changes" after choosing.
                            </small>
                        </div>
                    </div>
...
<style>
    ...
    /* ===== Left column: logo card + cover card stacked ===== */
    .profile-left {
        display: flex;
    ...
    /* layout ke .panel-card ki height:100% ko override karta hai */
    .profile-left .logo-card {
        height: auto !important;
        flex: 0 0 auto;
    }
 
    /* bachi hui jagah cover card le leta hai, isliye neeche khali space nahi rehta */
    .profile-left .cover-card {
        height: auto !important;
        flex: 1 1 auto;
    }
        ...
        if (coverInput) {
            coverInput.addEventListener('change', function() {
                if (this.files && this.files[0]) {
                    ...
                    if (fileSize > 4) {
                        alert('Cover image must be less than 4MB. Current: ' + fileSize.toFixed(2) + 'MB');
                        this.value = '';
                    ...
                    if (!validCoverTypes.includes(file.type)) {
                    ...
                    coverFileName.textContent = '📷 ' + file.name + ' (' + fileSize.toFixed(2) + ' MB)';
 
                    const coverReader = new FileReader();
                    coverReader.onload = function(e) {
                        coverPreview.innerHTML = `<img src="${e.target.result}" alt="Salon Cover Image">`;
                    };
