...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-1" style="color:#333;font-family:'Playfair Display',serif;"><i class="fas fa-heart me-2" style="color:#FF6B9D;"></i>Saved Salons</h4>
        <p style="color:#aaa;font-size:0.85rem;margin:0;">{{ $favorites->total() }} salons saved</p>
    </div>
    <a href="{{ route('salons.index') }}" class="btn btn-sm rounded-pill px-4" style="background:#FF6B9D;color:#fff;border:none;font-weight:600;">
...
<div class="row g-4">
        ...
        <div class="bg-white rounded-4 overflow-hidden" style="border:1px solid #fce4ec;transition:all .3s;" onmouseover="this.style.transform='translateY(-6px)';this.style.boxShadow='0 15px 40px rgba(255, 107, 157,0.15)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'">
            <div style="height:180px;overflow:hidden;position:relative;">
                ...
                <form action="{{ route('client.favorites.toggle',$salon->id) }}" method="POST" style="position:absolute;top:12px;right:12px;">
                    ...
                    <button style="width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,0.9);border:none;display:flex;align-items:center;justify-content:center;cursor:pointer;" title="Remove from favorites">
                        <i class="fas fa-heart" style="color:#FF6B9D;font-size:0.9rem;"></i>
                    </button>
                </form>
            ...
            <div class="p-4">
                ...
                <div style="color:#888;font-size:0.82rem;margin-bottom:0.75rem;">
                    <i class="fas fa-map-marker-alt me-1" style="color:#FF6B9D;"></i>{{ $salon->area ? $salon->area.', ' : '' }}{{ $salon->city }}
                </div>
                <div class="d-flex flex-wrap gap-1 mb-3">
                    @foreach($salon->services->take(3) as $service)
                    <span style="background:#fff0f7;color:#FF6B9D;border:1px solid #fce4ec;padding:2px 8px;border-radius:10px;font-size:0.72rem;font-weight:500;">{{ $service->name }}</span>
                    @endforeach
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('salons.show',$salon->slug) }}" class="btn btn-sm flex-grow-1" style="background:#fff0f7;color:#FF6B9D;border:1px solid #fce4ec;border-radius:10px;font-weight:600;">
                        <i class="fas fa-eye me-1"></i>View Salon
                    </a>
                    <a href="{{ route('booking.step1',$salon->id) }}" class="btn btn-sm flex-grow-1" style="background:#FF6B9D;color:#fff;border:none;border-radius:10px;font-weight:600;">
    ...
    <div class="col-12">
        <div class="text-center py-5 bg-white rounded-4" style="border:2px dashed #fce4ec;">
            <i class="fas fa-heart fa-4x mb-3" style="color:rgba(255, 107, 157,0.2);"></i>
            <h5 style="color:#333;">No saved salons yet</h5>
            <p style="color:#aaa;">Tap the heart icon on any salon to save it here</p>
            <a href="{{ route('salons.index') }}" class="btn rounded-pill px-5 mt-2" style="background:#FF6B9D;color:#fff;border:none;font-weight:600;">
