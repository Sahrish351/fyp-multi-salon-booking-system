    ...
    <div class="booking-layout">
        ...
        <div class="sidebar">
            <div class="summary-card">
                ...
                <div id="bookingSummaryRows" style="font-size:0.82rem; color:#555;">
...
function loadSlots(date) {
    ...
    const slotUrl = `{{ url('/booking/slots') }}/${salonId}?stylist_id=${stylistId}&service_id=${serviceId}&date=${date}`;
    ...
    .then(async response => {
        ...
        if (!response.ok) {
            throw new Error(data.error || 'Server error occurred while loading slots.');
        }
        return data;
    })
    .then(data => {
        if (data.holiday) {
            grid.innerHTML = '<div style="grid-column:1/-1; text-align:center; color:#e11d48; padding:16px 0; font-size:0.85rem;"><i class="fas fa-calendar-times me-1"></i> Stylist is on holiday for this date.</div>';
            return;
        }
        ...
        if (data.slots && data.slots.length > 0) {
            ...
            data.slots.forEach(s => {
                ...
                const slotVal = s.id;
                const slotLbl = s.label;

                html += `<div class="time-slot ${statusClass}" data-time="${slotVal}"
                             ${s.available ? `onclick="selectTime(this, '${slotVal}', '${slotLbl}')"` : ''}>
                            ${slotLbl}
                        </div>`;
            });
            grid.innerHTML = html;
        } else {
            grid.innerHTML = '<div style="grid-column:1/-1; text-align:center; color:#e11d48; padding:16px 0; font-size:0.82rem;"><i class="fas fa-info-circle me-1"></i> No slots available for this date. You can join the waitlist below!</div>';
        }
    })
    .catch(error => {
        console.error("Slot Fetch Error:", error);
        grid.innerHTML = `<div style="grid-column:1/-1; text-align:center; color:#e11d48; padding:16px 0; font-size:0.82rem;"><i class="fas fa-exclamation-triangle me-1"></i> ${error.message}</div>`;
    });
}

function selectTime(el, slotId, label) {
    ...
    document.getElementById('slotInput').value = slotId;
    document.getElementById('waitlistInput').value = '0';

    document.getElementById('sideTimeText').textContent = label;
    checkContinueButton();
}
