...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .svc-card .sc-info .sc-name {
            font-size: 1rem;
            font-weight: 600;
            color: #1a1a1a;
            margin-bottom: 3px;
        }
        .svc-card .sc-info .sc-duration {
            font-size: 0.82rem;
            color: #888;
            margin-bottom: 3px;
        }
        .svc-card .sc-info .sc-price {
            font-size: 1rem;
            font-weight: 700;
            color: #E91E8C;
        }
        ...
        @media(max-width:576px) {
            ...
            .cat-chip { font-size: 0.78rem; padding: 6px 16px; }
            .svc-card { padding: 12px 14px; }
            .svc-card .sc-info .sc-name { font-size: 0.9rem; }
            .svc-card .sc-info .sc-price { font-size: 0.9rem; }
            .booking-wrapper { padding-left: 14px; padding-right: 14px; padding-bottom: 85px; }
            .salon-summary { padding: 12px 14px; }
            .salon-summary img { width: 44px; height: 44px; }
            .selected-services-box { padding: 14px; min-height: 80px; }
    ...
    <div class="booking-layout">
        ...
        <div class="left-panel">
            ...
            <div class="cat-scroll">
                ...
                @php
                    $categories = App\Models\Category::where('is_active', 1)->take(8)->get();
                @endphp
            ...
            {{-- Services by Category (Max 5 per category) --}}
            @php
                $categories = App\Models\Category::where('is_active', 1)->take(8)->get();
            @endphp
            @foreach($categories as $category)
                @php
                    $catServices = $services->where('category_id', $category->id)->take(5);
                @endphp
...
<script>
    ...
    function toggleService(card) {
        ...
        if (selectedServices[id]) {
            ...
            btn.outerHTML = '<button class="add-btn" id="btn-' + id + '" onclick="event.stopPropagation(); toggleService(this.closest(\'.svc-card\'))">+</button>';
        } else {
