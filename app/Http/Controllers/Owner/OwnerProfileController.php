    ...
    public function update(Request $request)
    {
        try {
            ...
            if ($request->hasFile('logo')) {
                ...
                $data['logo'] = $request->file('logo')->store('salon-logos', 'public');
            }
 
            
            if ($request->hasFile('cover_image')) {
                
                $oldCover = $salon->getRawOriginal('cover_image');
 
                if ($oldCover
                    && !str_starts_with($oldCover, 'http')
                    && Storage::disk('public')->exists($oldCover)) {
                    Storage::disk('public')->delete($oldCover);
                }
 
                $data['cover_image'] = $request->file('cover_image')->store('salon-covers', 'public');
            }
